...
def parse_delta(text: str) -> list[dict]:
    """2023 format: Name  Remuneration ($)  Expenses ($) — no car allowance or total cols.
    Elected officials only; stop when employee names appear."""
    # Collapse OCR spaces in numbers
    text = re.sub(r"(?<=\s)(\d)\s+(\d{1,3},\d{3})", r"\1\2", text)
    ...
    for line in text.splitlines():
        ...
        if not line:
            continue
        # Stop at first employee-only row (no position label, very long name list)
        if re.match(r"^(Total|Name|Remuneration|Expenses|CITY|SCHEDULE|FOR THE|Attachment|Page \d)", line, re.IGNORECASE):
            continue
        m = re.match(
            r"^([\w\u00C0-\u024F',\.\-][\w\u00C0-\u024F',\.\-\s]+?)\s+"
            r"(Mayor|Councillors?)\s+"
...
def parse_kamloops(text: str) -> list[dict]:
    ...
    Handles both 'Councilor' (one l) and 'Councillor' spellings.
    2023: columns are Remuneration | Taxable benefits | Total Income | Expenses | Council Representation"""
    # Collapse OCR spaces in numbers
    text = re.sub(r"(?<=\s)(\d)\s+(\d{1,3},\d{3})", r"\1\2", text)
    text = re.sub(r"(\d)\s+,(\d{3})", r"\1,\2", text)
    rows = []
    for line in text.splitlines():
        m = re.match(
            r"^([\w\u00C0-\u024F',\.\-\(\)][\w\u00C0-\u024F',\.\-\(\)\s]+?)\s+"
            r"(Mayor|Councilors?|Councillors?)\s+"
            r"\$?\s*(\d[\d,]+)"          # remuneration
            r"(?:\s+\$?\s*[\d,]+)?"      # taxable benefits (skip)
            r"(?:\s+\$?\s*[\d,]+)?"      # total income (skip)
            r"\s+\$?\s*(\d[\d,]*)"       # expenses
            r"(?:\s+\$?\s*[\d,]*)?",     # council representation (optional, skip)
...
def parse_new_westminster(text: str) -> list[dict]:
    """2023 format: Council Member  Remuneration  Transitional Allowance  Total  [expense cols...]
    Uses Total col (3rd number) as remuneration; sums expense cols. Highest paid = Mayor."""
    text = re.sub(r"(?<=\s)(\d)\s+(\d{1,3},\d{3})", r"\1\2", text)
    text = re.sub(r"(\d)\s+,(\d{3})", r"\1,\2", text)
    rows = []
    for line in text.splitlines():
        line = line.strip()
        if re.match(r"^(Total|Council Member|UBCM|FCM|Schedule|City|Transitional|Remuneration|$)", line, re.IGNORECASE):
            continue
        m = re.match(
            r"^([\w\u00C0-\u024F',\.\-][\w\u00C0-\u024F',\.\-\s]+?)\s+"
            r"(\d[\d,]+|-)"
        ...
        if not is_valid_name(name):
            continue
        rem = clean_amount(m.group(4))
        exp_tokens = re.findall(r"\d[\d,]*", m.group(5))
...
def parse_chilliwack(text: str) -> list[dict]:
    """Position FirstName LastName  Bylaw Rate  Other  Mileage  Expenses
    Remuneration = col 1 + col 2. OCR spaces collapsed before parsing."""
    # Collapse OCR spaces inside numbers: "1 34,559" -> "134,559", "4 30.99" -> "430.99"
    text = re.sub(r"(?<=\s)(\d)\s+(\d{1,3},\d{3})", r"\1\2", text)
    text = re.sub(r"(\d)\s+,(\d{3})", r"\1,\2", text)
    text = re.sub(r"(?<=\s)(\d)\s+(\d{2,3}\.\d{2})(?=\s|$)", r"\1\2", text)
...
def parse_victoria(text: str) -> list[dict]:
    """COUNCIL MEMBER  Remuneration  Taxable Benefits  Expenses.
    No position labels; highest paid = Mayor.
    Collapses OCR spaces in decimal amounts e.g. "2 06.25" -> "206.25"."""
    text = re.sub(r"(?<=\s)(\d)\s+(\d{1,3},\d{3})", r"\1\2", text)
    text = re.sub(r"(\d)\s+,(\d{3})", r"\1,\2", text)
    text = re.sub(r"(?<=\s)(\d)\s+(\d{2,3}\.\d{2})(?=\s|$)", r"\1\2", text)
    text = re.sub(r"(?<=\s)(\d)\s+(\d{2,3})(?=\s|$)", r"\1\2", text)
...
def parse_maple_ridge(text: str) -> list[dict]:
    ...
    OCR spaces in amounts e.g. '5 ,643.88' -> '5,643.88'."""
    # Collapse OCR spaces inside numbers
    text = re.sub(r"(?<=\s)(\d)\s+(\d{1,3},\d{3})", r"\1\2", text)
    text = re.sub(r"(\d)\s+,(\d{3})", r"\1,\2", text)
    rows = []
    for line in text.splitlines():
...
def parse_vancouver_page(page) -> list[dict]:
    """Bbox word-group parser for Vancouver 2023.
    Columns: Name  Remuneration(1)  Local Expenses(2)  Travel(3)  Discretionary(4).
    Each character is a separate word token due to extreme OCR spacing.
    Expenses = cols 2+3+4. Highest paid = Mayor."""
    ...
    for top in sorted(rows_by_top):
        if top < 110 or top > 260:
            continue

        row_words = sorted(rows_by_top[top], key=lambda w: w["x0"])
        joined = " ".join(w["text"] for w in row_words)

        if re.search(r"(MAYOR AND COUNCILLORS|REMUNERATION AND EXPENSES|Travel|Local|Discret)", joined, re.IGNORECASE):
            continue

        # Split at first $ or digit token after x0=150
        first_num_idx = None
        for idx, w in enumerate(row_words):
            if re.match(r"^[\$\d]", w["text"]) and w["x0"] > 150:
                first_num_idx = idx
                break
        ...
        name_words = [w["text"] for w in row_words[:first_num_idx]]
        num_words  = row_words[first_num_idx:]

        raw_name = "".join(name_words)
        raw_name = re.sub(r",([A-Z])", r", \1", raw_name)

        # Join all numeric chars without spaces, then extract comma-grouped numbers
        # e.g. ["$","2","0","0",",","9","3","7"] -> "$200,937" -> 200937
        num_raw = "".join(w["text"] for w in num_words)
        num_raw = re.sub(r"\$", " ", num_raw)  # treat $ as separator
        # Extract valid comma-formatted numbers: e.g. "200,937" or plain integers
        nums = re.findall(r"\d{1,3}(?:,\d{3})+|\d+", num_raw)
        if not nums:
            continue

        amounts = [clean_amount(n) for n in nums]
        # Filter out obvious non-amounts (single digits, very small numbers from OCR fragments)
        amounts = [a for a in amounts if a is not None and a >= 100]
        ...
        rem = amounts[0]
        exp_parts = amounts[1:]
        exp = sum(v for v in exp_parts if v is not None) if exp_parts else 0
...
# ...existing code...
    ...
    if re.search(r"CITY OF RICHMOND", text, re.IGNORECASE):
        return "Richmond"
    # Standard watermark: first non-empty line is "Municipality — 2023"
...
def extract_all(merged_pdf: str = "app/remuneration_schedules_2023.pdf") -> pd.DataFrame:
...
PARSERS = {
    ...
    "Sidney":          parse_sidney,
    "Surrey":          parse_surrey,
    "Vancouver":       parse_standard,  # bbox parser used directly
    "Victoria":        parse_victoria,
}


def extract_from_pdf(pdf_path, municipality: str) -> list[dict]:
    parser = PARSERS.get(municipality, parse_standard)
    with pdfplumber.open(pdf_path) as pdf:
        text = pdf.pages[0].extract_text() or ""
    return parser(text)
...
def _municipality_from_watermark(text: str) -> str | None:
    """Extract municipality name from watermark line e.g. 'Abbotsford — 2023'.
...
if __name__ == "__main__":
    ...
    manual_path = Path("data_prep/manual_remuneration_2023.csv")
    if manual_path.exists():
        ...
        if not manual.empty:
            ...
            manual["expenses"] = manual["expenses"].fillna(0)
    ...
    out = Path("data_prep/remuneration_2023.json")
